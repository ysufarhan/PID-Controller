...
class MotorPIDController:
    def __init__(self, master):
        ...
        self.rpm_data = []
        self.setpoint = 0
        self.window_size = 50  # Number of data points displayed on the plot
    ...
    def update_plot(self):
        ...
        if len(self.time_data) > self.window_size:
            time_window = self.time_data[-self.window_size:]
            rpm_window = self.rpm_data[-self.window_size:]
        else:
            time_window = self.time_data
            rpm_window = self.rpm_data

        self.line.set_data(time_window, rpm_window)
        self.ax.relim()
